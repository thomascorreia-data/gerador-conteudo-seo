...
- **Dois revisores em sequência, barato primeiro:**
  ...
  - `revisor_ia` — só roda se o `revisor` já tiver aprovado (não gasta uma chamada de IA em texto que já ia ser reprovado de graça). Julga TOM e REGRAS PROIBIDAS "em espírito" — pega paráfrase de coisa banida, elogio genérico, atribuição de qualidade indevida à empresa etc., que o determinístico não detecta por não bater a string exata. Usa o próprio template de geração (categoria x tom) como as regras, então cobre qualquer categoria sem duplicar lista nenhuma.
