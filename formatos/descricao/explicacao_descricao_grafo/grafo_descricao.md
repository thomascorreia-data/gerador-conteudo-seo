...
- **Só a coleta é ramificada por categoria** (`coletar_cidade`, `coletar_empresa`, `coletar_ponto_turistico`, `coletar_terminal_rodoviaria`, `coletar_lugar_generico`, `coletar_evento`) — cada uma chama seu `base_*.py` correspondente. `lugar_generico` e `evento` são as coletas mais simples: só Wikipédia, sem geocodificação (ver [`estrategias/lugar_generico.md`](../estrategias/lugar_generico.md) e [`estrategias/evento.md`](../estrategias/evento.md)). Categorias sem coletor implementado (`estado`, `país`) caem em `categoria_nao_implementada`, que só levanta `NotImplementedError` (a aresta pro `__end__` existe só porque todo nó do LangGraph precisa levar a algum lugar — na prática a exceção sobe antes disso).
